...
// Universal Conversation Bot
exports.universalConversationBot = onRequest({ 
  ...
  try {
    ...
    logger.info(`Universal Conversation Bot - Customer: ${customer}, Message: ${message}`);
    ...
    // Use AI to handle the conversation flow
    ...
    logger.error("Error in Universal Conversation Bot", err);
...
2. AFTER BARBER SELECTION:
   ...
   A) Booking: "Book me for 15:00 today"
      - Validate availability
      - Create booking
      - Use format: "BOOKING: SERVICE_ID:TIME"
   
   B) Availability: "When are you free today?"
      - Check working hours + existing bookings
      - Return free time slots
   
   C) Working Hours: "What are your working hours on Friday?"
      - Fetch hours from database
      - Respond with schedule
   
   D) Business Status: "Are you open now?"
...
7. Handle "yes" responses for booking confirmations
8. For product business requests, politely redirect: "I currently only handle barber shop appointments. Please contact the business directly for product inquiries."
...
async function saveConversation(customer, twilioNumber, message, reply, date, tz, type, barberId = null) {
  try {
    await admin.firestore().collection("conversations").add({
      ...
      timestamp: Date.now(),
      platform: 'whatsapp',
      botType: 'universal-conversation',
